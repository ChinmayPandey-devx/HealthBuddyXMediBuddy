...
import { CHAT_RESPONSES, CHAT_DEFAULT_RESPONSE } from '../data';

export default function ChatAssistant() {
  const [history, setHistory] = useState([
    { role: 'assistant', content: 'Hi Raj! I am HealthBuddy. How can I help you understand your metrics today?', isNew: false }
  ]);
  ...
  const handleSend = (text) => {
    ...
    setInput('');
    const newHistory = [...history, { role: 'user', content: userMessage, isNew: true }];
    setHistory(newHistory);
    setIsLoading(true);

    // Simulate 800ms typing indicator delay before starting typewriter
    setTimeout(() => {
      let matchedResponse = CHAT_DEFAULT_RESPONSE;
      ...
      setHistory([...newHistory, { role: 'assistant', content: matchedResponse, isNew: true }]);
      setIsLoading(false);
    }, 800);
  };
    ...
    <div className="flex flex-col h-full bg-gray-50 absolute inset-0">
            ...
            <div className={`flex gap-2 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div className={`p-3 rounded-2xl text-sm shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] leading-relaxed ${
                ...
                {msg.role === 'assistant' && msg.isNew ? (
                  <Typewriter text={msg.content} delay={0} speed={18} />
                ) : (
      ...
      <div className="p-3 bg-white border-t border-gray-100 shrink-0 pb-20">
        <div className="flex gap-2 overflow-x-auto mb-3 pb-1 no-scrollbar">
          <button onClick={() => handleSend("Is my BP dangerous?")} className="shrink-0 bg-blue-50 text-brand-blue border border-blue-100 px-3 py-1.5 rounded-full text-xs font-semibold hover:bg-blue-100 transition-colors">
            Is my BP dangerous?
          </button>
          <button onClick={() => handleSend("What should I eat today?")} className="shrink-0 bg-blue-50 text-brand-blue border border-blue-100 px-3 py-1.5 rounded-full text-xs font-semibold hover:bg-blue-100 transition-colors">
